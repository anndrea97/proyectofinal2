...
Se va a realizar un análisis del tipo de cambio EUR/USD - Dólar estadounidense del último mes. Como variables principales se cuentan con el valor del tipo de cambio en su apertura, su máximo, su mínimo, la apertura y el último valor. 
En un primer caso se muestra una tabla con los datos principales y las variables de dicho estudio.
...
A continuación, se realizan distintos gráficos para mostrar el avance del tipo de cambio. En el primer GGPlot, se enseñan los registros del tipo de cambio, se muestra la evolución del valor mínimo en el (eje y), la fecha se encuentra en el eje x. Con este gráfico se quiere demostrar que a finales de octubre el tipo de cambio alcanza su mayor mínimo en este útimo mes.
...
ggplot(eur, aes(x = Fecha, y = Máximo, col = "Cambio")) +
  geom_smooth(method ='loess', formula = y ~ x) + geom_line(colour = "blue")+ theme_clean() 
```

## Gráfico dinámico<a name="id3"></a>


Por último, el gráfico dinámico muestra la evolución del mínimo respecto a los días, 
```{r, echo=FALSE, warning= FALSE}
